...
// Use EEPROM to track program number across reboots
#include <EEPROM.h>
...
// global variable to hold program number
int prog;
...
//string_group.AddString(&lights_2);
  ...
  // Read program number from EEPROM
  Serial.begin(115200);
  Serial.println("HELLO");
  prog = EEPROM.read(0);
  Serial.print("EEPROM prog=");
  Serial.println(prog);
  if ( prog == 255 ) {
    prog = 0;
  }
  // Save value for next time
  EEPROM.write(0,(prog+1) % PROGRAM_COUNT);

//  Start from scratch on next restart
//  EEPROM.write(0,255);
  
  // Force the program we want...
  runner.disable_time_based_switching();
  runner.switch_program_to(prog);
...
//  runner.switch_program_to(prog);
  ...
  // Make sure we get the same program again...
//  runner.switch_program_to(prog);
